...
<div class="wrap">
    ...
    <?php
    		...
    		$sql="SELECT name, winrate,pick,ban FROM single_champion_stats WHERE role='top' ORDER BY winrate DESC";
    		...
    		echo "<table><tr><th>Top</th><th>Jungle</th><th>Paired Win rate</th><th>Synergy</th><th>Win rate</th><th>Pick rate</th><th>Ban Rate</th></tr>";
    		$sql_pair = "SELECT pair.T, pair.J, pair.paired_win, pair.S, single_champion_stats.winrate, single_champion_stats.pick, single_champion_stats.ban FROM (SELECT top_jungle_matchups.top As T, top_jungle_matchups.jungle AS J, top_jungle_matchups.winrate AS paired_win, top_jungle_matchups.synergy AS S FROM top_jungle_matchups, (SELECT top_jungle_matchups.top AS top, MAX(top_jungle_matchups.winrate) AS max_win FROM top_jungle_matchups GROUP BY top_jungle_matchups.top) AS temp WHERE top_jungle_matchups.top = temp.top AND top_jungle_matchups.winrate = temp.max_win) AS pair, single_champion_stats WHERE pair.J = single_champion_stats.name AND single_champion_stats.role = 'jungle' ORDER BY single_champion_stats.winrate";
    		$result_pair = $conn->query($sql_pair);
    		while($row = $result_pair->fetch_assoc()){
    			echo "<tr><td>".$row["T"]."</td><td>".$row["J"]."</td><td>".$row["paired_win"]."</td><td>".$row["S"]."</td><td>".$row["winrate"]."</td><td>".$row["pick"]."</td><td>".$row["ban"]."</td></tr>";
    		}
    		echo "</table>";
    ?>
         <!-- Optional JavaScript -->
